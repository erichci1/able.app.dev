...
export default async function AudioPage({ searchParams }: { searchParams?: Promise<SP> }) {
    ...
    async function fetchFallback() {
        let qy = supabase
            .from("audio")
            .select("id, title, summary, audio_url, created_at")
            .order("created_at", { ascending: !!sortAsc });
        if (q) qy = qy.ilike("title", `%${q}%`);
        return await qy;
    }
            ...
            <section className="card" style={{ padding: 16 }}>
                ...
                {error ? (
                    ...
                    <div className="muted" style={{ marginTop: 6 }}>
                        {phase === "all" ? "No audio yet." : `No ${cap(phase)} audio yet.`}
                        {q ? ` Matching “${q}”.` : ""}
                    </div>
                ) : (
